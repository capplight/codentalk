      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Ему было семь, и ездить он ещё не умел: I couldn't ride a bike.",
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Она спрашивает: Could you get dressed alone at five?",
          ],
        },
        {
          id: "rasskaz-i-vopros",
      ...
      blocks: [
        ...
        },
        {
          ...
          text:
            ...
            "выходило, — по couldn't.\n\nПричину называют словом because или " +
            "после двоеточия.",
        },
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- итог 4 ----
      {
        ...
        kind: "choice",
        outcome: "находить в рассказе о детстве, что человек умел, а что нет",
        // Строка была дословно из рассказа для чтения того же модуля. Взята новая
        // строка того же жанра, слова — карточки этого же модуля.
        prompt:
          "В рассказе строка: I could sing every pop song, and my friends could not. Что она говорит?",
        // Правка дошла до английской строки и не дошла до русских вариантов: в
        // строке про пение, а варианты говорили о знании, и третий поминал записи —
        // след прежней строки про книгу с динозаврами. Нашёл методист.
        options: [
          { text: "Автор мог спеть любую поп-песню, а друзья — нет.", correct: true },
          { text: "Друзья пели лучше автора." },
          { text: "Автор пел только по субботам." },
        ],
        hint: "Вторая половина строки повторяет слово умения с отрицанием.",
        why:
          "Автор мог, а друзья нет. Оборот could not в конце и говорит, что " +
          "у друзей это не выходило. Слово every здесь про песни, а не про дни.",
      },
      {
        id: "q-chto-sleduet-iz-stroki-rasskaza",
        kind: "choice",
        outcome: "находить в рассказе о детстве, что человек умел, а что нет",
        // Строка была дословно из рассказа для чтения того же модуля — шестнадцать
        // слов подряд. Нашёл методист, вторым проходом. Взята новая строка того же
        // жанра, слова — карточки этого же модуля.
        prompt:
          "В рассказе строка: I couldn't find my costume, so I went to the party as a clown. Что из неё следует?",
        options: [
          { text: "Автор не пошёл на праздник." },
          { text: "Автор надел не тот наряд, что собирался.", correct: true },
          { text: "Наряд клоуна автору нравился больше." },
        ],
        hint: "Смотри, что стоит после слова so.",
        why:
          "Надел не тот наряд. Слово so и вводит то, чем обернулась пропажа " +
          "костюма.",
      },
      ...
      // ---- итог 6 ----
      {
        ...
        outcome:
          "проверять рассказ о детстве: назван возраст, умение и то, что не выходило",
        // Рассказ повторял строку из текста для чтения того же модуля — семь слов
        // подряд. Взят другой рассказ, слова — карточки этого же модуля.
        prompt:
          "Рассказ: At six I could kick a ball. My father watched me every evening. The park was small. Какой из четырёх нужных строк здесь нет?",
        options: [
        ...
        why:
          "Той, что называет неудачу. Возраст назван, отец назван, а слова " +
          "couldn't нет ни разу.",
      },
      {
