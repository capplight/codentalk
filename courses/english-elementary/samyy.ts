 ...
 *    the при форме выделения. ОСТОРОЖНО: графа начинается словом «Can», то есть
 *    разрешает, а не обязывает. Писать «the обязательно» нельзя — та же
 *    подкатегория даёт «FORM: WITH 'MY' OR 'YOUR'» («I bought these clothes for
 *    my youngest brother») и A1 «FORM: 'MY BEST FRIEND'». На этом месте 30
 *    августа 2026 методист нашёл выдуманный запрет: урок 1 писал
 *    «обязательно», а урок 5 того же модуля учит my best friend.
 *
...
const module: Module = {
  ...
  sources: [
    {
      ...
      section:
        ...
        "ПРО WORST ОТДЕЛЬНО: слова worst нет в примерах ни одной графы A2, но " +
        "`worst (adj)` стоит в словнике A2 Key — значит берётся по мерке ступени. " +
        "Модуль даёт его карточкой рядом с `best`, и работа спрашивает его один " +
        "раз (`q-forma-ot-bad-est`). Графы, запрещающей worst, не существует. " +
        "ЗАПИСЬ ИСПРАВЛЕНА 30 августа 2026: здесь стояло «заданий на нём не " +
        "строит» — это осталось от прежней редакции и спорило с шапкой того же " +
        "файла. Нашёл методист",
      license: "внутреннее использование, публично не называем",
    },
    {
      ref: "Council of Europe, CEFR Companion Volume 2020",
      section:
        "УРОК ПИСЬМА «Пишем заметку о самом». с. 66, шкала «Overall written " +
        "production», графа A2: «Can produce a series of simple phrases and " +
        "sentences linked with simple connectors like \"and\", \"but\" and " +
        "\"because\"». " +
        "ШКАЛА ИСПРАВЛЕНА 30 августа 2026 по разбору методиста. Прежняя опора " +
        "называла «Notes, messages and forms», с. 84, — а эта шкала вся о записке " +
        "ЧЕЛОВЕКУ: её собственное описание на с. 83 говорит «transactional " +
        "interactive writing… taking or leaving messages… to friends, service " +
        "people, teachers». Наш урок просит написать заметку О ГОРОДЕ, то есть " +
        "текст для читателей, а не сообщение адресату. Та же подстановка была в " +
        "модуле 17, он её у себя нашёл и просил разобрать соседей: модуль 14 " +
        "оказался в порядке (там записка Алиму), модуль 15 — нет",
    ...
    },
    {
      ...
      section:
        "Карточки модуля взяты из словника ступени, кроме одной. Подбор шёл через " +
        "`npm run slovo -- --zapas`. " +
        "ИСКЛЮЧЕНИЕ — `mosque`, И ОНО ОБЪЯВЛЯЕТСЯ. Слова нет в словнике A2 Key " +
        "(искал построчно по всему разбору; `church` там тоже нет), а Oxford 5000 " +
        "даёт «mosque n. B2». То есть ни одно из двух оснований ступени его не " +
        "покрывает. Слово оставлено как решение преподавателя: курс написан для " +
        "Казахстана, и мечеть — обычное здание города, о котором ученик говорит " +
        "первым делом. Найдено методистом 30 августа 2026; прежняя опора " +
        "утверждала «все карточки из словника», и это была неправда. Вопрос " +
        "вынесен владельцу: подтвердить или заменить на здание из словника " +
        "(`hotel`, `library`, `museum`, `station`) — слово работает в семи местах. " +
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          why:
            "Верно первое и третье. Шумнее всех попугаи, а самое маленькое животное — " +
            "паук.",
        },
        {
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          why:
            "The south. Юг назван самым жарким прямо в заметке.",
        },
        {
      ...
      blocks: [
        ...
        },
        {
          ...
          text:
            "My city is the oldest in the country.\nThe quietest side of it is behind the market, and the noise level there is the lowest.\nThe most beautiful place is the old square.\nMy best photo is from the top of the hill.",
          explain:
            "Круг назван в первой строке — in the country, — и дальше повторять " +
            "его не нужно: он понятен из неё. Последняя строка берёт my вместо the.",
        },
        {
        ...
        },
        {
          ...
          why:
            ...
            "с вещью, а не в конце строки. Вынести его в начало — In the city the " +
            "quietest place… — тоже верно, и такой ответ засчитывается.",
        },
        {
  ...
  quiz: {
    ...
    questions: [
      ...
      {
        // Строка была дословно из заметки для чтения того же модуля. Первая правка
        // взяла слонов и кроликов — и заметка тут же спорила: там самый большой
        // слон, а самый маленький паук. Нашёл методист. Заметка теперь о другом
        // месте, и качества взяты те, каких у заметки о зоопарке нет. Имя вопроса
        // тоже было прежним — про самого тихого.
        id: "q-kto-samyy-shumnyy",
        kind: "short",
        outcome: "находить в тексте, что самое большое и самое старое",
        prompt:
          "В заметке о парке строка: The kittens are the youngest animals here, and the ducks are the noisiest. Кто самый шумный? Ответь одним словом по-английски.",
        answer: "ducks",
        accept: ["Ducks", "the ducks", "duck"],
        hint: "Слова youngest и noisiest значат разное.",
        why:
          "The ducks. Слово noisiest называет самых шумных, а котята, наоборот, " +
          "самые молодые.",
      },
      ...
      },
      {
        ...
        why:
          "Нет, если ясно из заметки. Дальше круг понятен из первой строки, и " +
      ...
      },
      {
        ...
        options: [
          ...
          { text: "Окончания -est у big." },
        ],
        hint: "Форма выделения собрана верно — не хватает другого.",
        why:
          "Круга. Форма the biggest собрана правильно, но без круга «самый " +
          "большой» значит что угодно: среди парков города, страны или мира.",
      },
    ],
  },
};
