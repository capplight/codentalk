 ...
 *
 * ЧТО ЗДЕСЬ ЖДЁТ СЛОВА ВЛАДЕЛЬЦА. Урок 1 говорит, что `quite` перед словами
 * вроде `good`, `expensive`, `old` слабее, чем `very`, `really` и `so`. Ни один
 * источник в `materials/` силу этих слов не размечает: графа зовёт их всех
 * adverbs of degree и на этом останавливается. Основание в уроке — словарное
 * соответствие «quite = довольно», то есть решение преподавателя, а не выписка.
 *
