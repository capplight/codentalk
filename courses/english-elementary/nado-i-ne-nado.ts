 ...
 *
 * ОТСЮДА ПРИВЫЧКА, КОТОРУЮ МОДУЛЬ ДЕРЖИТ В УРОКАХ О ЛИЦАХ: о себе и о
 * собеседнике годятся оба оборота, о другом человеке модуль берёт `has to`.
 * Ученику это даётся случаем, а не запретом: «о нём говорят так».
 *
 * СЛОВО «ТОЛЬКО» ОТСЮДА СНЯТО 30 августа 2026, и это вторая правка одной и той
 * же ошибки. Здесь стояло «о третьем лице — ТОЛЬКО has to», а графа
 * «MODALITY must, A2, USE: OBLIGATION AND NECESSITY» говорит «Can use 'must'
 * with a limited range of pronouns ('I' and 'you')» — это разрешение, а не
 * запрет, и запрещающей графы нет.
 *
 * Модуль сам этот запрет и нарушал — двенадцать раз: `Every pupil must show an
 * ID at the door` в тексте чтения и в работе, `Every pupil must bring a camera
 * to the club` в таблице и образце письма, `My brother must show his ID`
 * ответом вопроса работы. Первым это заметил модуль 26 и записал у себя
 * («в модуле 25 я вычитал у графы must запрет, которого там не было»), а до
 * самого модуля 25 правка не дошла. Нашёл методист общим проходом.
 *
 ...
 * ВТОРАЯ половина нормы — пятнадцать НОВЫХ для ступени слов: новых тут девять
 * из одиннадцати, потому что `passport` и `machine` Oxford помечает A1. Счёт
 * поправлен 30 августа 2026: здесь стояло «семь», и цифра не пережила
 * добавления карточек `dive` и `against`.
 *
 ...
 *
 * СЕМЬ СЛОВ ВЫШЕ СТУПЕНИ: `set`, `outdoor`, `indoor` — B1; `licence`, `pupil`,
 * `id`, `kit` — B2. Все семь стоят в словнике A2 Key.
 *
 * `outdoor` И `indoor` ДОПИСАНЫ ОБЩИМ ПРОХОДОМ 29 АВГУСТА 2026. Сверка нашла,
 * что задание урока 4 требует слова `outdoor` («the outdoor pool»), а карточки
 * ему нет ни в одном модуле ступени: модуль 21 даёт НАРЕЧИЕ `outdoors`, а это
 * другая форма и другое слово. Тем же уроком спрашивалось и `indoor`. Оба
 * слова взяты по мерке ступени и стоят в уроке 3 — раньше, чем впервые
 * понадобились. Проверено ПОСТРОЧНО по
...
const module: Module = {
  ...
  sources: [
    {
      ...
      section:
        ...
        "вовсе — отсюда привычка модуля: о другом человеке он берёт has to. Это " +
        "не запрет: графа `must` разрешает узкий круг лиц, а запрещающей графы " +
        "нет, и сам модуль пишет `Every pupil must show an ID` в тексте чтения. " +
    ...
    },
    {
      ...
      section:
        ...
        "ученик школы. " +
        "СЕМЬ СЛОВ ВЫШЕ СТУПЕНИ, И ЭТО ОБЪЯВЛЯЕТСЯ НАРОЧНО: set, outdoor, " +
        "indoor — B1; licence, pupil, id, kit — B2. Все семь стоят в словнике " +
        "A2 Key. Проверено ПОСТРОЧНО. " +
        "`outdoor` И `indoor` ДОПИСАНЫ ОБЩИМ ПРОХОДОМ 29 АВГУСТА 2026: сверка " +
        "нашла, что задание урока 4 требует слова `outdoor`, а карточки ему нет " +
        "ни в одном модуле ступени. Модуль 21 даёт наречие `outdoors` — другая " +
        "форма и другое слово. Словник даёт оба: `indoor (adj)`, `outdoor " +
        "(adj)`. " +
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "И есть второе слово об обязанности — must; оно короче, и меняется не так.",
          ],
        },
        {
        ...
        },
        {
          ...
          text: [
            ...
            "О другом человеке так и говорят: he has to show his ID.",
          ],
        },
        {
          id: "ne-he-must",
        ...
        // ---- задания ----
        {
          ...
          answer: "has to",
          hint: "У he и she первое слово меняется: has to.",
          why:
        ...
        },
        {
          ...
          hint: "После must слово to не ставят, а у he и she стоит has to.",
          why:
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "На машине висит табличка: You mustn't use the machine.",
            "Слово mustn't значит запрет, а не совет: так не разрешено никому.",
          ],
        },
        {
          id: "nado-i-nelzya",
        ...
        },
        {
          ...
          items: [
            ...
            { term: "outdoor", translation: "уличный, под открытым небом", example: "The outdoor pool is closed today.", hint: "/ˌaʊtˈdɔː/" },
            { term: "indoor", translation: "тот, что в помещении", example: "Please use the indoor pool.", hint: "/ˈɪndɔː/" },
            { term: "camera", translation: "фотоаппарат", example: "Every pupil must bring a camera to the club.", hint: "/ˈkæmərə/" },
          ],
        },
      ...
      blocks: [
        ...
        },
        {
          ...
          text: [
            ...
            "Спроси себя: этого делать нельзя — или просто не обязательно?",
            "Нельзя — mustn't. Не обязательно, но можно — don't have to.",
          ],
        },
        {
          id: "ne-dont-must",
      ...
      blocks: [
        {
          ...
          kind: "explain",
          text: [
            "Ученик написал правила клуба: «Be good. Don't be bad.» — «Ведите себя хорошо. Не ведите себя плохо.»",
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- итог 4 ----
      {
        // Тот же контраст на том же полотенце, что в уроке 5, — ученик узнавал пару
        // строк, а не разбирал их. Первая правка сделала из этого «исправь и
        // запиши» — и вопрос стал близнецом соседнего, следующего сразу за ним.
        // Нашёл методист. Теперь спрашивают, что гостю делать с вещью.
        id: "q-chto-delat-s-kepkoy",
        kind: "choice",
        outcome: "сказать, что делать не обязан: You don't have to bring a kit",
        prompt: "Гость прочитал: You don't have to bring a cap. Что ему делать с кепкой?",
        options: [
          { text: "Оставить дома: приносить нельзя." },
          { text: "Взять с собой, если хочет, — или не брать.", correct: true },
          { text: "Купить кепку на месте." },
        ],
        hint: "Один оборот запрещает, другой снимает обязанность.",
        why:
          "Как хочет. Оборот don't have to говорит, что кепка не нужна, но своей " +
          "никто не запрещает. Запрет — это mustn't. По-русски обе строки часто " +
          "переводят одинаково, и это сбивает.",
      },
      {
      ...
      // ---- итог 5 ----
      {
        ...
        kind: "short",
        outcome: "находить в правилах, что обязательно, а что нет",
        // Строка была дословно взята из текста для чтения того же модуля: урок
        // чтения переигрывался второй раз. Строка новая, того же жанра.
        prompt:
          "В правилах строка: Every guest must wash before the pool. Кто обязан это делать? Ответь одним словом по-английски.",
        answer: "guest",
        accept: ["Guest", "every guest", "guests"],
        hint: "Ответ стоит в самом начале строки.",
        why:
          "Guest. Слово every перед ним говорит, что правило касается каждого " +
          "без исключения.",
      },
      {
        ...
        kind: "choice",
        outcome: "находить в правилах, что обязательно, а что нет",
        // Строка была дословно из текста для чтения того же модуля. Первая правка
        // взяла строку про бег — и отвлекающий «бегать запрещено» оказался ВЕРНЫМ
        // по тем же правилам: там стоит You mustn't run indoors. Работа наказывала
        // за верный ответ. Нашёл методист. Взят билет: о нём правила молчат.
        prompt:
          "В правилах строка: You don't have to show a ticket: the pool is free for pupils. Что из этого следует?",
        options: [
          { text: "Билет показывать запрещено." },
          { text: "Билет надо купить на месте." },
          { text: "Билет не нужен: ученикам вход бесплатный.", correct: true },
        ],
        hint: "Вторая половина строки объясняет первую.",
        why:
          "Билет не нужен. Оборот don't have to снимает обязанность, а запрет ставят " +
          "другим оборотом — mustn't.",
      },
