  ...
  sources: [
    ...
    },
    {
      ...
      section:
        ...
        "время ученику. Второе: строка «Им же говорят о том, что бывает всегда» " +
        "стоит потому, что источник даёт в одной строке и «habits», и «future " +
        "meaning»; без неё ученик свяжет имя только с расписанием. " +
        "ПОЧЕМУ ИМЯ ЗДЕСЬ, А НЕ В МОДУЛЕ 2. Настоящее время мелькает в модуле 2 " +
        "столбцом сравнения («I work» рядом с «I worked»), но предметом урока оно " +
        "там не является, а в том же абзаце уже названо Past Simple. Два имени " +
        "разом ученик не разложит. Модуль 11 — первый, где это время предмет. " +
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          head: ["По-английски", "По-русски", "Когда так говорят"],
          rows: [
            [
              "I am going to cook dinner.",
            ...
            [
              "I will cook dinner.",
              "Я приготовлю ужин.",
              "берётся за дело, обещает",
            ],
            [
              "I am cooking dinner for Dana at six.",
              "В шесть я готовлю ужин Дане.",
              "договорились, и Дана этого ждёт",
            ],
            [
              "The train leaves at six.",
              "Поезд уходит в шесть.",
              "так в расписании, от людей не зависит",
            ],
          ],
        },
        {
          ...
          kind: "explain",
          text: [
            "Все четыре строки об одном вечере, и все четыре верны.",
            "Различать их проще всего по русскому переводу: «собираюсь готовить», «приготовлю», «готовлю», «уходит».",
            "Правила «здесь только так» тут нет.",
            "На «Приходи в шесть» можно ответить и I'll come, и I'm coming — годятся обе.",
          ],
        },
        {
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- итог 6 ----
      {
        ...
        kind: "choice",
        outcome: "находить нужное время в заметке о поезде",
        // Строка была дословно из заметки для чтения того же модуля — девять слов
        // подряд. Строка новая, того же жанра; о цене вечернего поезда заметка
        // модуля не говорит, так что спор с ней исключён.
        prompt:
          "В заметке написано: «The evening train costs less than the morning one.» Что из этого видно?",
        options: [
          { text: "Вечерний поезд дороже утреннего" },
          { text: "Вечернего поезда больше нет" },
          { text: "Вечерний поезд дешевле утреннего", correct: true },
        ],
        why: "costs less — стоит меньше, значит вечерний дешевле утреннего.",
      },
      {
