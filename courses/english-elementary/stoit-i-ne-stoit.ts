      ...
      blocks: [
        {
          ...
          text: [
            ...
            "По-русски говорят «не стоит», по-английски — You shouldn't eat that.",
            "Слово совета то же, только с отрицанием.",
          ],
        },
        {
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "По-русски это «стоит ли мне взять карту?».",
            "По-английски — Should I take a map?",
          ],
        },
        {
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          hint: "Строка, где зовут вместе, начинается с why, а за ним идёт don't we.",
      ...
      blocks: [
        {
          ...
          kind: "explain",
          text: [
            "Ученик написал другу: «Come. It is nice here.» — «Приезжай. Здесь хорошо.»",
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- итог 5 ----
      {
        ...
        prompt:
          "В письме строка: You shouldn't go windsurfing today because it is too windy. Что советует автор?",
        options: [
          { text: "Сегодня кататься лучше не стоит.", correct: true },
          { text: "Кататься стоит только в ветер." },
          { text: "Кататься сегодня запрещено." },
        ],
        hint: "Слово shouldn't — совет, а не запрет.",
        why:
          "Сегодня лучше не стоит. Про запрет сказали бы mustn't.",
      },
      {
        id: "q-najti-predlozhenie-v-pisme",
        kind: "short",
        outcome: "находить в советах приезжему, что стоит делать, а что нет",
        // Строка была дословно из письма для чтения того же модуля. Строка новая,
        // того же жанра: market — карточка модуля 1 ступени.
        prompt:
          "В письме строка: Why don't we go to the market on Sunday? Что делает автор этой строкой? Ответь одним словом по-русски.",
        answer: "зовёт",
        // Урок 5 того же модуля зовёт это действие «предлагает», а работа такого
        // ответа не принимала. Нашли оба проверяющих.
        accept: ["Зовёт", "зовет", "приглашает", "предлагает", "предлагает поехать"],
        // Прежняя подсказка содержала ответ дословно: «автор зовёт поехать вместе».
        hint: "Смотри на слово we: то, что названо, делают вдвоём.",
        why:
          "Зовёт. Оборот why don't we значит «а почему бы нам не» — так зовут " +
