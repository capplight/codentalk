 ...
 *
 * Программа: docs/programma-english-elementary.md, модуль 9 — «брать на себя
 * дело»; грамматика: Future Simple — `will`, `won't`; отрицание и вопрос.
 *
 ...
 *
 * СВЯЗЬ С МОДУЛЕМ 8 ЕСТЬ, А ПРИВЫЧНОГО ПРАВИЛА ПРИ НЕЙ НЕТ — 30 августа 2026.
 *
 * Урок 1 показывает `going to` и `will` рядом, потому что ученик только что
 * прошёл первый и обязан понять, зачем ему второй. Напрашивалось учебниковое
 * правило: «going to — решил заранее, will — решаю сейчас». **Источник его не
 * подтверждает.** В `English Grammar Profile Online.csv` оба описаны ОДНИМИ И
 * ТЕМИ ЖЕ словами:
 *
 * - `Can use 'will' and ''ll' to talk about plans and intentions.`
 * - `Can use the affirmative and question forms of 'be going to' talk about
 *   plans and intentions.`
 *
 * Различие по времени принятия решения — учебниковый обычай, и графы его не
 * дают. Дать его правилом значило бы объявить ошибкой верные строки: та самая
 * порода, на которой курс горел трижды (CLAUDE.md, «Графа — это разрешение»).
 *
 * Поэтому урок 1 говорит только то, что источник даёт: обе строки верны, а
 * разницу показывает РУССКИЙ перевод — «собираюсь ответить» против «отвечу».
 * Это факт о русском языке, а не утверждение об английском. И добавлено то,
 * что у `will` в источнике есть отдельно: `Can use 'will' to express
 * willingness`, `Can use 'will' to talk about willingness and offers`, `Can use
 * 'will' to make requests` — обещание, предложение помощи и просьба. Про
 * `going to` при этом ничего не отрицается.
 *
 * ИМЯ ВРЕМЕНИ — `FUTURE SIMPLE`, И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА ОТ 22 АВГУСТА 2026.
 *
 * Спор пришлось решать ему как преподавателю: источники разошлись между собой, а
 * такого расхождения ни один скрипт не разрешает. Урок 1 называет имя один раз,
 * при первой встрече, и без объяснений, зачем название нужно.
 *
 * Осторожно: сначала здесь стояло «имени нет по той же причине, что в модуле 8»,
 * и это было неверно. Нашёл методист. У `will` имя в источниках ЕСТЬ, а у
 * оборота `going to` его действительно нет:
 *
 * - `English Grammar Profile Online.csv`, подкатегория `future simple (with
 *   will and shall)` — 16 строк, и это имя. Модуль стоит ровно на ней: см.
 *   опоры ниже, A1, «FORM: AFFIRMATIVE 'WILL'». Для сравнения, у оборота
 *   подкатегория зовётся `future with be going to` (14 строк) — описание;
 * - но тот же файл держит `MODALITY will` — ещё 13 строк, то есть кладёт `will`
 *   и в модальность тоже;
 * - а руководство A2 Key, с. 52, перечень Tenses, имени не даёт вовсе: «Future
 *   with will and shall: offers, promises, predictions, etc.». Руководство
 *   Young Learners, с. 55 и 77, так же: «Will  Will you do your homework this
 *   evening?» — форма, а не время.
 *
 * Один предмет, два разных решения у источников — значит вопрос к владельцу как
 * к преподавателю (CLAUDE.md, «Как править содержание»). Данных о том, «как
 * принято», наши источники не содержат вовсе, и вывести ответ из них нельзя.
 *
 * Владелец выбрал `Future Simple`: имя стоит в English Grammar Profile дословно,
 * и оно же самое ходовое в учебниках — ученик узнает его где угодно. Расхождение
 * с руководством A2 Key при этом остаётся и здесь записано нарочно, чтобы
 * следующий проверяющий не принял имя за мою выдумку и не снял его.
 *
 * Модуль 8 имени НЕ получает, и это не непоследовательность: у оборота `going
 * to` имени нет ни у одного из трёх источников, а здесь оно есть у одного из
 * них. Разбор — в шапке `ya-sobirayus.ts`.
 *
 ...
 *
 * ОБОРОТ `LET'S` ВЗЯТ ЗДЕСЬ РАНЬШЕ СВОЕГО МОДУЛЯ, И ЭТО ОБЪЯВЛЯЕТСЯ. Урок 6
 * (чтение) держит строку «I won't be free on Thursday, so let's meet on Friday»,
 * и оборот глоссирован прямо в тексте. Учит ему модуль 12, тремя модулями
 * позже; модуль 8 от него отказался нарочно и написал об этом в своей шапке.
 * Ступени оборот не превышает: `let (v)` стоит в словнике A2 Key, а English
 * Grammar Profile даёт `FORM/USE: 'LET'S', SUGGESTION` на A2. Здесь он не
 * предмет урока, а слово в чужом письме — ученик его узнаёт, а не производит.
 * Найдено методистом при общем проходе 30 августа 2026.
 *
  ...
  sources: [
    {
      ...
      section:
        ...
        "ИМЯ ВРЕМЕНИ. Имя стоит в самой подкатегории — `future simple (with will " +
        "and shall)`, 16 строк файла; отсюда строка урока 1 «Время, в котором " +
        "стоит I will reply, называется Future Simple». Осторожно: тот же файл " +
        "держит ещё 13 строк `MODALITY will`, а руководство A2 Key, с. 52, имени не " +
        "даёт вовсе — «Future with will and shall: offers, promises, predictions, " +
        "etc.». Расхождение источников разрешил владелец как преподаватель 22 " +
        "августа 2026: имя даём, оно самое ходовое в учебниках. Это решение, а не " +
        "вывод из источника, и снимать его нельзя. " +
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "I will reply tomorrow. — Я отвечу завтра.",
            "По-русски это одно слово — «отвечу». По-английски их два: will и сам глагол.",
            "Так дают обещание — берутся что-то сделать.",
            "Русское «отвечу» несёт в себе сразу две вещи: и то, что дело впереди, и само дело.",
            "Время, в котором стоит I will reply, называется Future Simple.",
          ],
        },
        {
          id: "will-po-licam",
          ...
          rows: [
            ["I", "I will arrange it.", "Я это устрою."],
            ["you", "You will arrange it.", "Ты это устроишь."],
            ["he", "He will arrange it.", "Он это устроит."],
            ["she", "She will arrange it.", "Она это устроит."],
            ["we", "We will arrange it.", "Мы это устроим."],
            ["they", "They will arrange it.", "Они это устроят."],
          ],
        },
        {
          ...
          text: [
            ...
            "В русском меняется само слово: устрою, устроишь, устроит, устроим, устроят.",
            "В английском не меняется ничего: will стоит при любом лице одно и то же.",
            "Запоминать тут нечего: will одинаково при всех лицах.",
        ...
        {
          id: "vtoroy-sposob-o-budushchem",
          kind: "table",
          caption: "Два способа сказать о будущем",
          zvuchat: ["I am going to reply tomorrow.", "I will reply tomorrow."],
          head: ["По-английски", "По-русски"],
          rows: [
            ["I am going to reply tomorrow.", "Я собираюсь ответить завтра."],
            ["I will reply tomorrow.", "Я отвечу завтра."],
          ],
        },
        {
          id: "chem-eti-dva-sposoba-raznye",
          kind: "explain",
          text: [
            "В модуле «Я собираюсь» был другой способ сказать о будущем — going to.",
            "Обе строки в таблице верны, и обе о завтрашнем дне.",
            "Разница видна в переводе: «собираюсь ответить» и «отвечу».",
            "«Собираюсь ответить» говорит, что человек так решил заранее. «Отвечу» просто называет дело: сказал — и сделает.",
            "Кроме того, will годится, чтобы пообещать, предложить помощь и попросить: об этом следующие уроки модуля.",
          ],
        },
        {
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          tone: "info",
          text:
            "Слово please значит «пожалуйста».\n\nВ строках этого урока оно " +
            "стоит в конце и отделяется запятой.",
        },
        {
          id: "prosby-na-rabote",
          kind: "example",
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- итог 4 ----
      {
        ...
        kind: "choice",
        outcome: "просить: Will you help me?",
        prompt: "Собеседник сказал: «Will you carry this box for me?» Что он сделал?",
        options: [
        ...
        why:
          "Попросил о помощи. Слово me и выдаёт просьбу: дело нужно самому " +
          "говорящему. Без него строка была бы и вопросом о планах.",
      },
      {
        id: "q-dopisat-me",
        kind: "gap",
