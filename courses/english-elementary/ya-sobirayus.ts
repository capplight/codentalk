 ...
 * going to».
 *
 * АНГЛИЙСКОГО ИМЕНИ ВРЕМЕНИ ЗДЕСЬ НЕТ, И ЭТО РЕШЕНИЕ, А НЕ ПРОПУСК. CLAUDE.md
 * требует назвать время английским именем при первой встрече — но у этой записи
 * такого имени источник не даёт. Руководство A2 Key, с. 52, перечень Tenses,
 * зовёт её описанием, а не именем: «Future with going to». Рядом там стоят
 * настоящие имена — «Past simple», «Present continuous», — и разница видна. Имя
 * «Future Simple» ученику дал бы не источник, а я; писать в уроке «оборот going
 * to» вернее, тем более что сама запись стоит по-английски в каждом примере.
 *
 * Методист подтвердил это тремя источниками сразу, и вот они, чтобы следующий
 * проверяющий не искал заново:
 *
 * - `English Grammar Profile Online.csv`: подкатегория зовётся `future with be
 *   going to` — описание. Рядом в том же файле стоит `future simple (with will
 *   and shall)`, и там имя есть. Разница видна в самом файле;
 * - руководство A2 Key, с. 52: у пяти времён после имени двоеточие («Past
 *   simple: past events»), а здесь имени нет вовсе — «Future with going to»;
 * - руководство Young Learners, с. 55 и 77, перечень нового на A2 Flyers: `Past
 *   continuous` названо именем, а `going to` дано формой — «Be going to  It
 *   isn't going to rain today».
 *
 * Поиск `future simple|future tense` по всем разборам PDF в `materials/.md/` и
 * `materials/.text/` не дал для `going to` ни одного совпадения.
 *
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "По-английски в вопросе слово переставляют.",
        ...
        },
        {
          ...
          text: [
            ...
            "По-английски к yes и no добавляют форму из вопроса.",
      ...
      blocks: [
        ...
        },
        {
          ...
          text: [
            ...
            "Отвечают на такой вопрос по существу: называют дело, место или время.",
      ...
      blocks: [
        ...
        },
        {
          ...
          text:
            "TRIP TO THE COUNTRYSIDE\nOur club is going to visit the countryside on Sunday.\nWe are going to be at the station at nine.\nI'm going to bring the tickets.",
          perevod: {
            "TRIP TO THE COUNTRYSIDE": "ПОЕЗДКА ЗА ГОРОД",
            "Our club is going to visit the countryside on Sunday.":
