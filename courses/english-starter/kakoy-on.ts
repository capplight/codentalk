...
const module: Module = {
  ...
  sources: [
    ...
    },
    {
      ...
      section:
        "УРОК ПИСЬМА «Пишем объявление о продаже». с. 67, шкала Creative writing, графа A1, дословно: «Can use simple words/signs and phrases to describe certain everyday objects (e.g. the colour of a car, whether it is big or small)». Источник называет прямо и цвет, и размер — оба урока модуля. " +
        "Там же, с. 66, шкала Overall written production, графа A1: «Can produce simple isolated phrases and sentences» — заголовок и три отдельные строки это и есть. " +
        "ОГОВОРКА О СОСЕДНЕЙ ГРАФЕ: с. 68, шкала Reports and essays, графы A1 и Pre-A1 — «No descriptors available». Рассуждения на ступени нет, поэтому письмо здесь — описание из трёх строк, а не сочинение. " +
        "ПОРОГ В ДЕВЯТЬ СЛОВ — НАШ, а не из источника: три коротких строки короче не выходят. Счёт слов задают руководства Cambridge, и только со ступени выше нашей — см. отдельную запись об A2 Key ниже. " +
  ...
  outcomes: [
    ...
    "проверять объявление о продаже: артикль перед названием вещи и форму be при прилагательном",
  ],
  ...
  lessons: [
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          tone: "info",
          text:
            "Very ставят перед прилагательным, а не перед названием предмета. «A very " +
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        // ---- задания ----
        {
          ...
          hint: "Слово but показывает, что второе предложение спорит с первым.",
          why:
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          hint: "Слушай слово между двумя предложениями: оно и говорит «но».",
    ...
    // =====================================================================
    // Урок письма. Пара к уроку чтения этого же модуля: там объявление о
    // продаже читают, здесь пишут своё.
    //
    // Опора — в `sources`: цвет вещи назван источником прямо, в шкале
    // Creative writing.
    // =====================================================================
    {
      slug: "pishem-obyavlenie",
      title: "Пишем объявление о продаже",
      estimatedMinutes: 12,
      // Итог назван по той половине, которую проверяет работа: письмо машина
      // не оценивает. То же решение, что в модуле 3.
      outcome: "проверять объявление о продаже: артикль перед названием вещи и форму be при прилагательном",
      ...
      blocks: [
        ...
        },
        {
          ...
          kind: "example",
          caption: "Объявление о продаже",
          // Образец нарочно не тот, что в уроке чтения этого модуля: там
          // продают велосипед за 80, здесь телефон за 20. Прежний образец
          ...
          explain:
            "Заголовок FOR SALE говорит, что вещь продают, — он знаком по уроку чтения. " +
            "Дальше строки по одной: вещь, прилагательное, цвет, цена. Слово but " +
            "соединяет два предложения, которые спорят друг с другом: «он старый» и " +
            "«он дешёвый».",
        },
        {
          id: "chto-teryayut-v-obyavlenii",
          kind: "note",
          tone: "mistake",
          text:
            "Теряют здесь два слова. Форму be: «It very good» вместо «It is very good». " +
            "И артикль в первой строке: «This is camera» вместо «This is a camera». Во " +
            "второй и третьей строках артикля нет вовсе — там после is стоит прилагательное, а " +
            "не название вещи.",
        },
        {
          id: "poryadok-priznakov",
          kind: "note",
          tone: "info",
          text:
            "Цвет можно поставить отдельной строкой, а можно рядом с другим " +
            "прилагательным: «It is old and black». Оба способа верны, и в уроке об " +
            "описании предмета цвет стоит рядом: It's black and very big. В объявлении " +
            "ниже мы ставим его отдельной строкой — так короче каждая строка.",
        },

        // ---- задания ----
        {
          // Случай взят не тот, что разобран во врезке выше («This is camera»):
          // иначе ответ читается из врезки. Нашёл методист.
          id: "z1-chto-poteryano-v-obyavlenii",
          kind: "choice",
          prompt: "В какой строке объявления ничего не потеряно?",
          // Верная строка нарочно не та, что стоит во врезке выше («This is a
          // camera»): иначе ученик находит её глазами, не читая двух других.
          // Прежняя правка перенесла совпадение с неверного варианта на верный —
          // нашёл методист.
          options: [
            { text: "It very expensive." },
            { text: "This is a bike.", correct: true },
            { text: "This is green camera." },
          ],
          hint: "Проверь в каждой строке два места: форму be и артикль перед названием.",
          why:
            "This is a bike. В первой строке потеряна форма be, в третьей — артикль " +
            "перед названием вещи.",
        },
        {
          id: "z2-ispravit-stroku-obyavleniya",
          kind: "short",
          prompt: "Исправь и запиши строку целиком: «It very expensive.»",
          answer: "It is very expensive.",
          // Полная и короткая записи обе верны, и урок сейчас не о выборе между
          // ними. Регистр и точку в конце сверка снимает сама.
          accept: ["It's very expensive."],
          hint: "Между It и словом very должна стоять форма be.",
          why: "It is very expensive. Годится и короткая запись — It's very expensive.",
        },
        {
          // Задание на письмо. Порог в девять словах — наш: три коротких
          // строки короче не выходят. Машина письмо не оценивает, показывает
          // образец и список для самопроверки.
          id: "z3-napisat-obyavlenie",
          kind: "essay",
          prompt:
            "Напиши объявление о продаже своей вещи: заголовок FOR SALE, потом что " +
            "продаётся и какое оно, потом цвет, потом цену.",
          // Порог в девять слов — наш: столько выходит в самом коротком верном
          // ответе («FOR SALE. A bag. It is new. It is black. Price: 30.» —
          // одиннадцать; девять оставлено с запасом на короткое объявление
          // вроде «A pen. It is new. It is red. Price: 5.»).
          minWords: 9,
          sample: "FOR SALE. A bag. It is new and very big. It is black. Price: 30.",
          checklist: [
            "Есть заголовок FOR SALE.",
            "Перед названием вещи стоит a или an.",
            "В строках о прилагательном и о цвете стоит форма be: is или it's.",
            "Цена написана цифрой после слова Price.",
          ],
          hint:
            "Возьми образец и подставь своё: FOR SALE. A … . It is … . It is … . " +
            "Price: … .",
          why:
            "Сравни свой ответ с образцом по четырём пунктам выше. Вещь, прилагательное, " +
            "цвет и цена у каждого свои. Важно другое: артикль стоит только перед " +
            "названием вещи, а форма be — в строках о прилагательном и о цвете.",
        },
      ],
    },
  ],
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- письмо -----------------------------------------------------
      // Угол другой, чем в уроке. Там ищут потерянное слово в готовой строке;
      // здесь строк три, и лишнее слово стоит в той, где его быть не должно, —
      // ошибка в самом артикле: перед гласным звуком нужен an. Прежний вопрос
      // брал выдуманную ошибку — «It is a new», артикль без названия вещи, — и
      // такого ученик не пишет. Нашёл методист.
      {
        id: "q-lishniy-artikl-v-obyavlenii",
        kind: "choice",
        // Итог назван по той половине, которую проверяет работа: письмо машина
      // не оценивает. То же решение, что в модуле 3.
      outcome: "проверять объявление о продаже: артикль перед названием вещи и форму be при прилагательном",
        prompt:
          "В объявлении написано: «This is a old camera.» Что здесь не так?",
        options: [
          { text: "Артикль здесь лишний" },
          { text: "Перед old нужен an, а не a", correct: true },
          { text: "Всё верно" },
        ],
        hint: "Слово old начинается с гласного звука. Какой артикль идёт перед такими?",
        why:
          "This is an old camera. Артикль нужен — за прилагательным стоит название вещи. " +
          "Но перед гласным звуком он другой: an.",
      },
    ],
  },
};
