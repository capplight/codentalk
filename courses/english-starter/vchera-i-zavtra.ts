...
const module: Module = {
  ...
  lessons: [
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Смотри на конец слова — так же, как в правиле про -ing. Разберём три случая.",
          ],
        },
        {
          id: "tablica-ed",
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        // Запись убрана: все четыре времени звучат в таблице выше, а рассказ — построчно.
        {
          ...
          tone: "info",
          text:
            "Это последний урок о временах: дальше в модуле остались чтение, слушание " +
            "и письмо.\n\nТеперь ты умеешь рассказать о себе, о своих делах, о " +
            "вчерашнем дне и о планах. С этого и начинается настоящий разговор.",
        },
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        // ---- задания ----
        {
          // Случай взят третий, не тот, что разобран во врезке выше: там стоят
          // «Yesterday I will come» и «Tomorrow I worked». Иначе ответ ученик
          // берёт из врезки, не читая строку. Нашёл методист.
          id: "z1-slovo-i-vremya",
          kind: "choice",
          prompt:
            "В открытке написано: «Now I worked in the garden.» Что здесь не так?",
          options: [
            { text: "Слово garden написано с маленькой буквы" },
            { text: "Слово времени говорит о сейчас, а глагол о вчера", correct: true },
            { text: "Всё верно" },
          ],
          hint: "Прочитай слово времени и глагол подряд: об одном ли они дне?",
          why:
            "Now I am working. Слово now о том, что идёт прямо сейчас, а worked — о " +
            "прошлом. Сойтись они не могут.",
        },
        {
          id: "z2-ispravit-vremya",
          kind: "short",
          // Условие называет, какую половину оставить. Без этого верных решений
          // два: можно исправить глагол, а можно слово времени. Нашёл методист.
          prompt:
            "Исправь и запиши предложение целиком: «Yesterday I will see my friend.» " +
  ...
  quiz: {
    ...
    questions: [
      ...
      // ---- письмо -----------------------------------------------------
      // Прежний вопрос объявлял ошибкой ПРАВИЛЬНУЮ открытку: все три строки в
      // ней сходились, а неверным считался порядок строк — правила о котором
      // в уроке нет и в источниках не нашлось. Ученик отвечал верно и получал
      // «пока не так». Нашёл методист.
      //
      // Угол другой, чем в уроке: там расходится пара «слово времени и
      // глагол» в одном предложении, здесь надо найти испорченную строку
      // среди трёх — две другие верны.
      {
        id: "q-najti-stroku-ne-po-vremeni",
        kind: "hottext",
        outcome: "проверять в открытке, что время глагола сходится со словом времени",
