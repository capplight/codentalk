...
const module: Module = {
      ...
      blocks: [
        {
          ...
          kind: "explain",
          text: [
            "«Ты студент?» — такой вопрос задают в первом же разговоре, и задавать его " +
              "приходится самому.",
  ...
  quiz: {
    ...
    questions: [
      ...
      {
        // Урочное задание отличалось одним местоимением при том же ответе is.
        // Теперь вопрос требует найти ошибку в чужом ответе, а не вписать форму.
        id: "q-otvet-da-o-ney",
        kind: "choice",
        outcome: "коротко отвечать «да»: Yes, I am, Yes, he is",
        prompt: "«Is she a nurse?» Ученица ответила: «Yes, she is a nurse.» Что не так?",
        options: [
          { text: "Форма be выбрана неверно." },
          { text: "Местоимение должно быть другим." },
          { text: "Краткий ответ длиннее нужного: занятие в нём не повторяют.", correct: true },
        ],
        hint: "Сравни с ответом на такой же вопрос о себе: Yes, I am.",
        why:
          "Краткий ответ кончается формой be: Yes, she is. Само занятие в нём не " +
          "повторяют — иначе ответ перестаёт быть кратким.",
      },
      {
        id: "q-otvet-net",
        kind: "short",
      ...
      {
        // Урочное задание отличалось одним местоимением при том же ответе isn't.
        // Теперь ответ надо собрать целиком, а не вписать одну форму.
        id: "q-otvet-net-o-nyom",
        kind: "order",
        outcome: "коротко отвечать «нет»: No, I'm not, No, she isn't",
        prompt: "Собери краткий ответ «нет» на вопрос «Is he from Spain?»",
        items: ["isn't.", "No,", "he"],
        answer: [1, 2, 0],
        hint: "Три части, и последняя — форма be с отрицанием.",
        why: "No, he isn't. Дальше в кратком ответе ничего не идёт.",
      },
      {
      ...
      },
      {
        ...
        kind: "short",
        outcome: "отличать вопрос от сообщения по порядку слов",
        // Все три варианта совпадали с урочными дословно, включая верный, и
        // условие отличалось одним местоимением. Теперь строку надо переписать.
        prompt:
          "Перед тобой сообщение: «She is from Almaty.» Сделай из него вопрос и запиши целиком.",
        answer: "Is she from Almaty?",
        exact: true,
        accept: ["Is she from Almaty"],
        why:
          "Is she from Almaty? Первые два слова меняются местами, и знак вопроса " +
          "встаёт в конец. Решает порядок слов, а не знак.",
      },
      {
