      ...
      blocks: [
        ...
        },
        {
          ...
          text: [
            ...
            //
            // 28 АВГУСТА СНЯТО ЕЩЁ ОДНО. Стояло «пара работает ТОЛЬКО с
            // вещами» — придуманный запрет: пример самого словаря Longman в
            // выписке о вежливости даёт «'Would you like some more coffee?'
            // 'No, thank you, I'm fine.'», а модуль 28 ступени Elementary
            // отвечает так и на приглашение к делу. Нашёл методист.
            "Пара Yes, please и No, thank you хорошо годится, когда предлагают вещь.",
  ...
  quiz: {
    ...
    questions: [
      ...
      },
      {
        ...
        kind: "choice",
        outcome: "понимать по объявлению, что делать и чего делать нельзя",
        // Строка «Don't open the window» была дословно из записи для слушания того
        // же модуля. Объявление новое, того же вида.
        prompt:
          "Объявление: «Please wait here. Don't close the door.» Человек хочет закрыть " +
          "дверь. Можно ли ему это сделать?",
        options: [
          { text: "Нельзя", correct: true },
          { text: "Можно" },
          { text: "Про дверь не сказано" },
        ],
        why:
          "Don't close the door. Строка с Don't и говорит, чего делать нельзя.",
      },
      {
      ...
      // ---- итог 3 ----
      {
        // Урочное задание отличалось одним делом при том же ответе please, а
        // условие вдобавок само называло ответ: «добавляешь „пожалуйста" в конце».
        // Теперь спрашивают о запятой — о том, что от места и зависит.
        id: "q-please-dopisat",
        kind: "choice",
        outcome: "ставить please в указании и просьбе",
        prompt: "Две записи: «Come here, please» и «Please come here». Чем они различаются?",
        options: [
          { text: "Первая вежливее второй." },
          { text: "Только местом please: в конце он идёт после запятой, в начале запятой нет.", correct: true },
          { text: "Вторая обращена к нескольким людям." },
        ],
        hint: "Сравни знаки в обеих записях.",
        why:
          "Только местом. Оба места верны, и выбирать можно любое; в конце please " +
          "отделяют запятой, в начале — нет.",
      },
      {
        id: "q-please-vybor",
        kind: "choice",
        outcome: "ставить please в указании и просьбе",
        // Задание урока 3 отличалось от этого одним словом — окно вместо двери — при
        // том же наборе ошибок. Теперь спрашивается не «как сказать», а что в чужой
        // строке стоит не на месте.
        prompt: "Ученик написал: Open please the door. Что в этой строке не на месте?",
        options: [
          { text: "Вежливое слово: в середине оно не стоит.", correct: true },
          { text: "Действие: оно должно стоять вторым." },
          { text: "Название вещи: перед ним нужен другой артикль." },
        ],
        hint: "Прочитай строку вслух и найди, что разрывает указание.",
        // Первая редакция этой правки писала «его место в конце» — выдуманный
        // запрет: урок 3 даёт please ДВА места, начало и конец, и прямо говорит
        // «оба места верны». Нашёл методист. Признак был налицо: «место в конце»
        // вместо «не в середине».
        why:
          "Вежливое слово. Мест у него два — начало и конец: Please open the door " +
          "или Open the door, please. В середине оно разрывает указание.",
      },
      {
        id: "q-please-napisat",
        kind: "short",
      ...
      // ---- итог 4 ----
      {
        // Задание урока 4 отличалось от этого одним словом при том же ответе Can.
        // Теперь спрашивается не первое слово, а почему просьба звучит мягче
        // указания, — это и есть предмет урока.
        // Задание урока 4 отличалось от прежнего вопроса одним словом при том же
        // ответе Can. Первая правка сделала из него вопрос «почему просьба мягче»
        // — и он проходился, не читая английского: подсказка указывала на
        // вопросительный знак, а верный вариант стоял дословно в объяснении урока.
        // Нашёл методист. Теперь указание надо переделать в просьбу самому.
        id: "q-peredelat-ukazanie-v-prosbu",
        kind: "short",
        outcome: "просить: Can you help?",
        prompt: "Указание звучит так: Wait here. Переделай его в просьбу со словом can.",
        answer: "Can you wait here?",
        accept: ["Can you wait here"],
        hint: "После can идёт тот, кого просят, и только потом дело.",
        why: "Can you wait here? Указание стало вопросом, и собеседник может ответить.",
      },
      {
        id: "q-prosba-vybor",
        kind: "choice",
