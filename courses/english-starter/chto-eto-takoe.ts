...
const module: Module = {
  ...
  sources: [
    {
      ...
      section:
        "УРОК ЧТЕНИЯ «Читаем объявление о находке». с. 56, шкала Reading for orientation, графа A1, дословно: «Can recognise familiar names, words/signs and very basic phrases on simple notices in the most common everyday situations». Объявление бюро находок — простое объявление обыденного случая, а названия вещей разобраны в уроках модуля. " +
        "Там же, с. 56, графа Pre-A1: «Can find information about places, times and prices on posters, flyers and notices» — ступень НИЖЕ нашей, значит поиск сведений по объявлению посилен наверняка. Здесь ищут место: в сумке или в коробке. " +
        "ЧТО СДЕРЖИВАЛО СОСТАВ ТЕКСТА: множественного числа курс на этом месте ещё не давал (оно в модуле 8), поэтому вещи названы по одной, с артиклем. Предлог on тоже не разобран (модуль 12), поэтому мест только два и оба с in. " +
  ...
  outcomes: [
    ...
    "находить в объявлении, какая вещь где лежит",
  ],
  ...
  lessons: [
    ...
    // =====================================================================
    // Урок чтения. Жанр — объявление бюро находок: короткие строки, в каждой
    // название вещи и место. Читается не подряд, а поиском: ученик ищет свою
    // вещь, а не читает список целиком.
    //
    // Новых английских слов урок не вводит. Опора — в `sources`.
    // =====================================================================
    {
      slug: "chitaem-spisok-veshchey",
      title: "Читаем объявление о находке",
      estimatedMinutes: 12,
      outcome: "находить в объявлении, какая вещь где лежит",

      blocks: [
        {
          id: "zachem-chitat-spisok-veshchey",
          kind: "explain",
          text: [
            "Объявление бюро находок читают не подряд. Человек ищет одну свою вещь и " +
              "смотрит только, названа она или нет.",
            "Строки в нём устроены одинаково: сначала место, потом двоеточие, потом " +
              "что там лежит. Место стоит первым — так его быстрее найти глазами.",
          ],
        },
        {
          ...
          body: [
            "In the bag: a book, a pen and a key.",
            "In the box: a phone and a ticket.",
            "Is this your bag? Is this your box?",
          ],
        },
        {
          id: "dvoetochie-v-spiske",
          kind: "note",
          tone: "info",
          text:
            "Двоеточие здесь работает как в бланке из первого модуля: слева подпись, " +
            "справа то, что к ней относится. Только там слева стояло поле, а тут — место.",
        },
        {
          id: "artikl-v-spiske",
          kind: "note",
          tone: "info",
          text:
            "Перед каждой вещью стоит a: a book, a pen, a key. В списке артикль не " +
            "пропадает — вещь называют так же, как в предложении.",
        },
        ...
        // ---- задания ----
        {
          ...
          prompt: "Где лежит билет?",
          options: [
            { text: "В сумке" },
            { text: "В коробке", correct: true },
            { text: "В объявлении билет не назван" },
          ],
          hint: "Ищи слово ticket и посмотри, в какой оно строке.",
          why: "In the box: a phone and a ticket. Билет назван во второй строке.",
        },
        {
          id: "z2-chto-eshchyo-v-sumke",
          kind: "short",
          about: "obyavlenie-o-nahodke",
          prompt:
            "В сумке лежат книга, ручка и ещё одна вещь. Напиши её название по-английски.",
          answer: "key",
          accept: ["a key", "Key"],
          hint: "Третья вещь в первой строке.",
          why: "A key. В первой строке названы три вещи: a book, a pen and a key.",
        },
        {
          id: "z3-otmetit-chego-net",
          kind: "hottext",
          about: "obyavlenie-o-nahodke",
          prompt: "Отметь вещи, которых в объявлении нет.",
          parts: [
            { text: "a map", selectable: true, correct: true },
            { text: " · " },
            { text: "a pen", selectable: true },
            { text: " · " },
            { text: "a car", selectable: true, correct: true },
            { text: " · " },
            { text: "a phone", selectable: true },
          ],
          hint: "Пройди по обеим строкам и вычёркивай найденное.",
          why:
            "A map и a car в объявлении не названы. Ручка стоит в первой строке, " +
            "телефон — во второй.",
        },
        {
          id: "z4-zachem-poslednyaya-stroka",
          kind: "choice",
          prompt: "Зачем в конце объявления стоит «Is this your bag? Is this your box?»",
          options: [
            { text: "Чтобы спросить, чьи это вещи", correct: true },
            { text: "Чтобы назвать ещё две вещи" },
            { text: "Чтобы повторить первые две строки" },
          ],
          hint: "Эта строка обращена к читающему, а не к вещам.",
          why:
            "Объявление ищет хозяина: первые две строки говорят, что нашли, последняя " +
            "спрашивает, не твоё ли это.",
        },
      ],
    },
  ],
  ...
  quiz: {
    ...
    questions: [
      ...
      // вещь названа ДВАЖДЫ, в разных местах, и надо заметить оба.
      {
        ...
        outcome: "находить в объявлении, какая вещь где лежит",
        prompt:
          "Объявление: «In the bag: a pen and a key.» / «In the box: a pen and a map.» " +
          "Где лежит ручка?",
        options: [
          { text: "Только в сумке" },
          { text: "И в сумке, и в коробке", correct: true },
          { text: "Только в коробке" },
        ],
        hint: "Дочитай обе строки до конца, а не останавливайся на первой находке.",
        why:
          "A pen названа в обеих строках. Найдя вещь в первой строке, список закрывают " +
          "рано — а она бывает и дальше.",
      },
    ],
  },
};
