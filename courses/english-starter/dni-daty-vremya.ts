  ...
  sources: [
    {
      ...
      section:
        ...
        "ПОРОГ В ДВЕНАДЦАТЬ СЛОВ — НАШ: столько выходит в трёх коротких строках, которых требует условие. Порог взят по условию, а не по образцу (в нём 19 слов), чтобы не отсекать короткий верный ответ. " +
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Поэтому в сообщении называют три вещи и все три подряд: день, время и место. " +
              "Пропустишь одну — придётся писать второе сообщение.",
          ],
        },
        {
          ...
          explain:
            "Первая строка спрашивает, свободен ли собеседник, вторая называет время, " +
            "третья — место встречи. День стоит с on, время с at, часть суток с in.",
        },
        {
          ...
          text:
            ...
            "Проверяй сообщение по этим трём словам, а не по смыслу.",
        },
        {
        ...
        // ---- задания ----
        {
          ...
          kind: "choice",
          prompt:
            "Человек написал: «Are you free in Friday? The meeting is at seven.» " +
            "Что не так в первом предложении?",
          options: [
            { text: "Перед днём недели нужен on", correct: true },
            { text: "Перед днём недели нужен at" },
            { text: "Всё верно" },
          ],
          hint: "Посмотри в таблицу трёх предлогов: с чем идёт in, а с чем on.",
          why:
            "Are you free on Friday? Предлог in идёт с месяцем и с частью суток, а с " +
            "днём недели — on.",
        },
        {
        ...
        },
        {
          ...
          // строках, которых требует условие. Образец длиннее (18 слов), и
          // порог нарочно взят по условию, а не по образцу.
          id: "z3-napisat-soobshchenie",
          kind: "essay",
          prompt:
            "Напиши сообщение о встрече тремя строками: спроси, свободен ли собеседник " +
            "в какой-то день, назови время и назови место.",
          minWords: 12,
          sample:
          ...
          checklist: [
            ...
            "Перед частью суток стоит in: in the morning, in the evening.",
  ...
  quiz: {
    ...
    questions: [
      ...
      // в сообщении три предлога, два верны, и надо найти третий.
      {
        ...
        why:
          "Перед часом ставят at: at six. Первый in стоит перед числом и потому неверен, " +
          "второй — перед частью суток и потому верен. On перед днём недели тоже верно.",
      },
    ],
  },
};
