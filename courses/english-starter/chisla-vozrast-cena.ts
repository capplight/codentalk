      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Сама цена по-английски — price: a good price.",
          ],
        },
        {
          id: "tablica-how-much",
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Это то же умение, что и диктовать имя по буквам из первого модуля: только " +
              "вместо букв — цифры.",
            "Сам номер по-английски зовут словом number: a room number, a flat number.",
          ],
        },
        {
          id: "tablica-nomerov",
        ...
        {
          // Решение владельца от 2 сентября 2026. Работа объявляла ошибкой
          // sixty-three для комнаты 63 — обычное английское чтение. Урок говорил
          // о ДЛИННОМ номере, а короткий не оговаривал вовсе, и правило можно было
          // прочесть шире, чем оно есть. Источники в materials/ этого не решают:
          // о чтении номеров они молчат.
          id: "korotkiy-nomer-celikom",
          kind: "note",
          tone: "info",
          text:
            "Короткий номер называют целиком.\n\nКомната 63 — это sixty-three. По " +
            "одной цифре диктуют длинный номер, где легко сбиться со счёта.",
        },
        {
          id: "zero-i-oh",
          kind: "note",
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          // Слово `lesson` вводится карточкой только в модуле 10, поэтому оно
          // объяснено здесь же. Слово `Age` из словарика убрано: модуль уже
          // дал его карточкой, и вторая подача сказала бы, что оно новое.
          // И то и другое нашёл методист.
          glossary: [
            { term: "lesson", translation: "занятие; во множественном — lessons" },
          ],
        },
        {
          // Правило тире-диапазона жило только в подсказке задания, а без него
          // строку Age прочитать нельзя. Поймал методист.
          id: "tire-mezhdu-chislami",
          kind: "note",
          tone: "info",
          text:
            "Тире между двумя числами значит «от … до …». Age: 15 — 20 — это от " +
        ...
        {
          // Было «какая цифра в телефоне идёт третьей» — задание проходилось
          // без единого английского слова и не проверяло свой итог: цифра
          // номера не цена и не количество. Поймал методист.
          id: "z5-cena-zanyatiya",
          kind: "short",
          about: "obyavlenie-o-gruppe",
          prompt:
            "Посмотри на объявление ENGLISH LESSONS. Сколько стоит одно занятие? " +
            "Ответь цифрой.",
          answer: "60",
          accept: ["sixty", "шестьдесят"],
          hint: "Найди строку со словом lesson.",
          why:
            "One lesson — 60. Шестьдесят. Числа 12 и 15 из других строк: одно про " +
            "учеников, другое про возраст.",
        },
      ],
    },
    ...
    // =====================================================================
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          // Оборот `there are` разбирается только в модуле 13, поэтому здесь
          // его нет: было «There are fourteen students», поймал методист.
          transcript: "My sister is thirteen. My brother is fourteen.",
        },
        ...
        {
          id: "z4-skolko-let-bratu",
          kind: "choice",
          about: "zapis-vozrast-i-chislo",
          prompt: "Послушай вторую запись. Сколько лет брату?",
          options: [
            { text: "40" },
          ...
          why: "My brother is fourteen. Четырнадцать: ударение на конце слова.",
        },
        {
          id: "z5-skazat-paru",
          kind: "speak",
  ...
  quiz: {
    ...
    questions: [
      ...
      },
      {
        ...
        // хотя итог — про чтение объявления. Заменено на чтение ценника.
        id: "q-cennik-skolko-vsego",
        kind: "short",
        outcome: "находить в объявлении цену и количество",
        prompt:
          "Ценник: «A pen — 3. A book — 12. Two pencils — 6.» Сколько стоит книга? " +
          "Ответь цифрой.",
        answer: "12",
        accept: ["twelve", "двенадцать"],
        why: "A book — 12. Двенадцать. Тройка — цена ручки, шестёрка — двух карандашей.",
      },
      {
      ...
      {
        // Три варианта из трёх и верный среди них стояли в задании 4 урока 1.
        // Теперь спрашивается не «в каком числе», а какая буква лишняя в чужом
        // чтении — и ответ надо написать.
        id: "q-nechitaemaya-bukva-v-two",
        kind: "short",
        outcome: "называть числа от одного до десяти",
        prompt:
          "Ученик прочитал число two как «твуу». Какую букву он произнёс зря? " +
          "Ответь самой буквой.",
        answer: "w",
        hint: "Произнеси слово вслух и сравни с тем, как оно написано.",
        why: "Буква w. В слове two её пишут, но не произносят: /tuː/.",
      },
      {
        id: "q-sobrat-13",
        kind: "choice",
      ...
      {
        id: "q-ispravit-zapis-nomera",
        kind: "short",
        outcome: "диктовать номер по одной цифре",
        // Прежний вопрос спрашивал про комнату 63 и объявлял ошибкой sixty-three
        // — обычное английское чтение двузначного числа. Урок говорит о ДЛИННОМ
        // номере, двузначный под это правило не подпадает, и ученик, ответивший
        // верно, получал «пока не так». Взят длинный номер и настоящая ошибка —
        // собрать первые две цифры в одно число.
        // Условие не говорило, чем разделять слова, а сверка тире не вырезает:
        // ученик, написавший «five zero eight», получал «пока не так». Нашёл
        // методист. Разделитель назван, как в задании 3 урока 6.
        prompt:
          "Ученик записал номер рейса 508 так: fifty — zero — eight. Запиши, как " +
          "этот номер диктуют: словами через дефис.",
        answer: "five — zero — eight",
        accept: ["five-zero-eight", "five - zero - eight", "five zero eight"],
        hint: "Цифр в номере три, значит и слов три.",
        why:
          "Five — zero — eight. Каждую цифру называют отдельно, поэтому 50 не " +
          "собирают в fifty.",
      },
      {
