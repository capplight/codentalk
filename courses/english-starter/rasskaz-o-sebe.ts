...
const module: Module = {
  ...
  sources: [
    ...
    },
    {
      ...
      section:
        ...
        "ПОРОГ В ОДИННАДЦАТЬ СЛОВ — НАШ, а не из источника: четыре коротких предложения короче не выходят. Счёт слов задают руководства Cambridge, и только со ступени выше нашей — см. отдельную запись об A2 Key ниже.",
  ...
  lessons: [
      ...
      blocks: [
        {
          ...
          text: [
            ...
            "Само занятие по-английски зовут словом job: What's your job? — I'm a driver.",
          ],
        },
        {
          id: "tablica-zanyatiy",
    ...
    // =====================================================================
    // ПЕРВЫЙ УРОК ПИСЬМА В КУРСЕ АНГЛИЙСКОГО. Сам вид задания `essay` в
    // проекте не новый: он с самого начала стоит в курсе по коду
    // (`courses/web-vvedenie/tekst-na-stranice.ts`).
    //
    ...
    // =====================================================================
    {
      ...
      // не оценивает, а обещать в итоге непроверяемое нельзя. Нашёл методист.
      outcome: "находить в записи о себе потерянные слова: форму be, артикль, заглавную букву",
      ...
      blocks: [
        ...
        },
        {
          ...
          // Приветствия в образце нет нарочно. Список для самопроверки требует
          // I'm в каждой строке и точку в конце — а «Hi!» не отвечает ни тому,
        ...
        {
          // Первое задание на письмо в курсе английского.
          //
          // Порог в одиннадцать слов — НАШ, а не из источника: ровно столько
          // выходит в четырёх коротких предложениях, которых требует условие.
          // Прежние двенадцать отсекали верный ответ — нашёл методист.
          //
  ...
  quiz: {
    ...
    questions: [
      ...
      {
        // Имя вопроса осталось от прежней редакции и говорило про Италию.
        id: "q-napisanie-francii",
        kind: "choice",
        outcome: "писать названия стран и городов с заглавной буквы",
      ...
      {
        // Пар было две: угадав одну, ученик получал вторую даром. И обе брались у
        // задания урока 8, где тот же вопрос стоит с тремя парами. Теперь пар три,
        // а сопоставляется не ответ с вопросом, а ответ с тем, о чём он говорит.
        id: "q-o-chem-otvet",
        kind: "match",
        outcome: "спрашивать, откуда собеседник, и отвечать на этот вопрос",
        prompt: "Сопоставь ответ и то, о чём он говорит.",
        left: ["I'm from Spain.", "I'm a nurse.", "I'm Dana."],
        right: ["занятие", "имя", "страна"],
        answer: [2, 0, 1],
        hint: "Смотри, что стоит после I'm: предлог, артикль или ничего из этого.",
        why:
          "I'm from Spain — страна, I'm a nurse — занятие, I'm Dana — имя. Страну " +
          "выдаёт предлог from, занятие — артикль перед ним, а перед именем не " +
          "ставят ни того, ни другого.",
      },
      {
