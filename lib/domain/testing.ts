...
export interface Question {
  ...
  topic: string;
  /**
   * Группа для равномерной выборки. У экзамена и работ частей это модуль.
   * Где её нет — выборка идёт по теме: у работы модуля так и надо.
   */
  group?: string;
 ...
 *
 * Берём по кругу из разных групп, а не подряд из одной: иначе работа проверяет
 * случайный кусок курса, а не курс.
 *
 * ЧТО ТАКОЕ ГРУППА И ПОЧЕМУ НЕ ТЕМА. Группировка по теме (итогу урока) годится
 * для работы модуля: там на один итог приходится несколько вопросов, и круг
 * даёт ровный охват.
 *
 * На экзамене ступени она не работала вовсе. Там у каждого из пятидесяти
 * вопросов свой итог — то есть пятьдесят групп по одному вопросу, — и круг
 * вырождался в случайную горсть. Считали: при тридцати вопросах из пятидесяти
 * примерно четыре модуля из двадцати пяти не проверялись в попытке никак, хотя
 * шапка экзамена обещает весь курс.
 *
 * Поэтому у вопроса есть отдельный признак группы. Для экзамена и работ частей
 * это модуль: круг берёт по вопросу из каждого модуля и только потом заходит на
 * второй заход. Ни один модуль не пропускается.
 */
export function selectQuestions<T extends { topic: string; group?: string }>(
  pool: T[],
  count: number,
  seed: string
): T[] {
  ...
  const klyuch = (q: T): string => q.group ?? q.topic;

  const byTopic = new Map<string, T[]>();
  for (const q of shuffle(pool, seed)) {
    const list = byTopic.get(klyuch(q)) ?? [];
    list.push(q);
    byTopic.set(klyuch(q), list);
  }
