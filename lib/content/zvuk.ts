 ...
 *
 * ОБРАЗЕЦ БЫВАЕТ РАЗГОВОРОМ, И ДО 12 сентября 2026 ОН ЧИТАЛСЯ ОДНИМ ГОЛОСОМ.
 * Ключ считался без признака разговора и без раскладки — `klyuchZvuka(fraza,
 * "slow")`, — а озвучка прямо ставила такому образцу `dvaGolosa: false`. Значит
 * `Does Aigerim dance? — Yes, she does.` синтез читал одним человеком, сам себе
 * отвечая. Решение владельца от 19 августа 2026 требует обратного: разговор
 * звучит на два голоса, где бы он ни стоял.
 *
 * Порода в проекте четвёртая по счёту и записана в CLAUDE.md трижды: так же
 * молчали примеры-разговоры сорока уроков, потом блоки `audio` целой ступени,
 * потом ряд букв через запятую. Всякий раз отчёты были чисты: имя файла
 * считается от текста, файл на месте, а звучит не то.
 *
 * ЦЕНА ПРАВКИ НАЗВАНА ВСЛУХ: имя файла сменилось у семи образцов курса, и
 * четыре из них в модуле 3, который стоит на боевом сайте. До переозвучки
 * (`npm run ozvuchka`) ученик получит там тишину, поэтому правка не выкладывается
 * без неё.
 */
export function adresObrazca(fraza: string, raskladka = ""): string {
  return adresZvuka("obrazec", klyuchZvuka(fraza, "slow", razgovorLi(fraza), raskladka));
}
