 ...
 * скороговоркой.
 *
 * РАЗДЕЛИТЕЛЕМ БЫВАЕТ И ЗАПЯТАЯ, И ЭТО НАШЛОСЬ ПОЗДНО. Владелец 5 сентября
 * 2026 попросил ставить в записях заданий запятые вместо дефисов («дефис
 * синтез читает слитно»), и с тех пор все ряды заданий записаны через запятую:
 * `C, A, T.`, `S, A, M.`, `A, U, R, O, R, A.`. А образец принимал ТОЛЬКО
 * точку — значит семь записей курса шли синтезу обычным текстом, без разметки
 * буквы звуком. Ровно то, на что владелец жаловался в тот же день: «C-A-T тут
 * A хрипит сильно, звучит как K» — одиночная `A` без разметки читается
 * безударным артиклем.
 *
 * Замысел урока 2 при этом прямо утверждал обратное — «там работает ветка
 * ryadBukv». Утверждение о коде, сделанное не прогоном, а чтением похожего
 * места. Нашёл второй методист чтением; поймать это отчётом было нечем: имя
 * файла считается от текста, а текст не менялся.
 */
...
function ssml(z: Zapis): string {
  ...
  if (ryadBukv(z.text) || diktovka(z.text)) {
    // Разделитель у ряда бывает и точкой, и запятой, у диктовки — дефисом.
