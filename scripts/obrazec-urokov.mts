 ...
 *
 * ЭТОТ ОБРАЗЕЦ УСТАРЕЛ 3 сентября 2026, В ТОТ ЖЕ ДЕНЬ. Формат владелец принял,
 * и пошаговый вид переехал в само приложение: `components/lesson/Shagi.tsx`,
 * `components/lesson/Ekran.tsx`, `components/lesson/UrokShagami.tsx`. Смотреть
 * теперь надо там — открыв урок курса, у которого объявлено `format: "shagi"`.
 *
 * Файл оставлен как снимок решения, а не как рабочий вид. Правки формата сюда
 * НЕ переносятся: два источника правды разъезжаются молча, и в этом проекте это
 * уже случалось не раз.
 *
 * Что здесь устарело сразу: таблица `KARTINKI` (значок теперь стоит в самом
 * содержании, поле `znak`) и то, что картинки тянутся с чужого адреса (в
 * приложении они лежат у нас, в `public/twemoji`).
 *
