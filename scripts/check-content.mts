...
/**
 * Курсы, замороженные решением владельца от 5 сентября 2026.
 *
 * Они не чинятся и доживают, пока новый курс их не заменит. Проверки по ним
 * гонять надо по-прежнему — они лежат в общем отчёте, — но находка в них не
 * ошибка сборки: красный отчёт по пятидесяти шести замороженным модулям закрыл
 * бы собой всё новое.
 */
const ZAMOROZHENY = new Set(["english-starter", "english-elementary"]);
...
function checkDubli(task: any, where: string): void {
  /*
   * ОТМЕЧАЕМЫХ ЧАСТЕЙ ЗДЕСЬ НЕТ, И ЭТО ИСПРАВЛЕНИЕ, А НЕ ПОБЛАЖКА.
   *
   * Проверка написана против списков, где одинаковые строки ломают задание:
   * два одинаковых варианта выбора, две одинаковые карточки сборки, два
   * одинаковых пункта столбца. Там повтор делает ответ неразличимым.
   *
   * У задания «отметь в тексте» повтор законен и даже неизбежен: ученик
   * отмечает МЕСТА, а не строки, и «отметь все формы be» в тексте из двух
   * предложений даёт два одинаковых слова. Верность считается по своему
   * `correct` у каждой части, значит неразличимости не возникает.
   *
   * Найдено на первом же задании нового формата. Пока таких заданий в курсе не
...
function checkPrivyazka(lesson: Lesson, where: string): void {
  ...
  /*
   * ЗАДАНИЕ БЕЗ `about` В УРОКЕ СЛУШАНИЯ СПРАШИВАЕТ ОБО ВСЕХ ЗАПИСЯХ РАЗОМ.
   *
   * Приписано 7 сентября 2026, на сборке модуля 11. Урок держит три записи и
   * задание «в какой из записей говорят о зонте» — чтобы ответить, ученик
   * обязан послушать все три. Поля `about` у такого задания нет и быть не
   * может: оно одно, а записей три.
   *
   * Без этой поблажки проверка звала две записи из трёх мёртвыми и была не
   * права: текст у них отняли, а взамен дали — просто не поодиночке.
   *
   * Поблажка узкая нарочно: она действует ТОЛЬКО когда в уроке правда есть
   * задание без `about`. Урок с тремя записями и тремя заданиями, каждое из
   * которых привязано к одной, проверяется по-прежнему построчно.
   */
  const estObshcheeZadanie = lesson.blocks.some(
    (b) => isTask(b) && !(b as { about?: string }).about
  );
...
function checkKartochkaRabotaet(mod: Module, where: string): void {
  ...
  for (const { slovo, urok } of kartochki) {
    ...
    //
    // Восклицательный знак снимается по той же причине, и его тут не было до
    // 29 августа 2026: карточки-связки вроде «What a pity!» в тексте урока
    // стоят с любым знаком после себя, а иногда и с тире. Проверка объявляла
    // такую карточку мёртвой, хотя связка работала строкой ниже.
 ...
 *
 * Нашлось 6 сентября 2026 при другой работе: в новом курсе лежит
...
/**
 * Подсказка у ВОПРОСА РАБОТЫ до ученика не доходит.
 *
 * `forBrowser` в `lib/content/quiz.ts` вычёркивает `hint` из всего, что уходит в
 * браузер, — вместе с `answer`, `accept`, `correct` и `why`. Комментарий там
 * говорит прямо: «подсказке в проверочной работе не место: она измеряет знания,
 * а не догадливость». Решение старое и верное; беда в том, что о нём не знал
 * никто, кроме самого файла.
 *
 * Поле `hint` у вопроса объявлено в типах наравне с полем `hint` у задания
 * урока — а там оно живое, его показывает `TaskCard`. Один и тот же ключ значит
 * в двух местах разное, и по виду кода это неотличимо.
 *
 * Цена: методисты и редакторы писали эти строки кругами и правили их по
 * разборам. Нашёл методист 6 сентября 2026, читая `lib/content/quiz.ts`, — а не
 * отчёт: ни одна проверка о мёртвом поле не спрашивала, и второй редактор в тот
 * же день назвал разнобой подсказок между работами двух модулей находкой, тогда
 * как прав был модуль с нулём.
 *
 * Ошибкой, а не сведениями: строка, которую никто не увидит, обманывает
 * следующего проверяющего — он читает её, оценивает и правит впустую.
 */
function checkMertvyePodskazki(course: Course): void {
  /*
   * Замороженным курсам — сведения, новому — ошибка.
   *
   * В `english-starter` и `english-elementary` таких подсказок 698. Курсы
   * заморожены решением владельца от 5 сентября 2026: они не чинятся и уйдут
   * под нож целиком, а красный отчёт по ним закрыл бы собой всё новое. Правило
   * то же, по которому здесь разведены старые и новые курсы у проверки голосов.
   */
  let mertvyh = 0;
  for (const bank of vseBanki(course)) {
    for (const q of bank.questions) {
      if (typeof q.hint !== "string" || !q.hint.trim()) continue;
      if (ZAMOROZHENY.has(course.slug)) {
        mertvyh += 1;
        continue;
      }
      fail(
        `${course.slug} → ${bank.gde} → ${q.id}`,
        "подсказка у вопроса работы не доходит до ученика — forBrowser вычёркивает hint; " +
          "убрать поле"
      );
    }
  }
  if (mertvyh) {
    warnings.push(
      `${course.slug}: ${mertvyh} подсказок в работах не доходят до ученика ` +
        `(forBrowser вычёркивает hint). Курс заморожен, потому сведения, а не ошибка`
    );
  }
}
 ...
 *
 * Нашёл сборщик 6 сентября 2026, внося в код работы модулей 2 и 5: четыре
 ...
 *
 * ЗАЧЕМ. Решение владельца от 19 августа: разговор звучит целиком и НА ДВА
 * ГОЛОСА, а реплики делятся по строкам примера. В коде это поле `razgovor:
 * true` и перевод строки между репликами; без них страница показывает весь
 * разговор одной строкой, а озвучка читает его одним голосом. Владелец однажды
 * уже назвал это скороговоркой — комментарий стоит в `components/lesson/
 * Material.tsx`.
 *
 * Ошибка не видна ни в одном отчёте: содержание целое, звук есть, ссылки на
 * месте. Видна она только на странице, глазами. Нашёл редактор в модуле 18, а
 * прогон по курсу дал ТРИДЦАТЬ ТРИ таких примера в семи модулях — все написаны
 * мною за последние дни. На ступени Beginner их ноль: там разговоры с самого
 * начала писались строками.
 *
 * Признак простой: тире с пробелами внутри текста примера. Именно по нему
 * `razgovorLi()` из `lib/content/zvuk.ts` делит реплики между голосами, и
 * именно оно остаётся, когда реплики не разнесли.
 */
function checkRazgovorRazneseyon(mod: Module, where: string): void {
  for (const lesson of mod.lessons) {
    for (const block of lesson.blocks) {
      ...
      /*
       * Тире внутри ОДНОЙ строки бывает и законным: у пар букв «G — J» на
       * ступени Beginner оно значит пару, а не смену говорящего. Слипшийся
       * разговор отличается тем, что весь пример — одна строка: переводов
       * строки в нём нет вовсе.
       */
      if (text.includes("\n")) continue;
      fail(
        `${where} → ${lesson.slug} → ${block.id}`,
        "разговор слипся в одну строку: тире с пробелами внутри текста примера. " +
          "Разнеси реплики по строкам (перевод строки вместо тире), поставь " +
          "`razgovor: true` и раздели `zvuchat` по репликам — иначе страница " +
          "покажет разговор одной строкой, а озвучка прочтёт его одним голосом"
      );
    }
  }
}
 ...
 *
 * ЧЕГО ЭТА ПРОВЕРКА НЕ ВИДИТ И НЕ УВИДИТ. Слово может стоять дважды в ОДНОМ
 * значении, а переводы при этом разойтись словами: `carefully` было «осторожно»
 * в модуле 7 и «внимательно, аккуратно» в модуле 17. Совпадения строк нет,
 * значит проверка молчит, — а ученику вторая карточка говорит, что слово новое.
 *
 * Расширять проверку на все расхождения нельзя: это те самые семьдесят пять
 * мест, где расхождение законно. Значит случай остаётся человеку. Найден он
 * общим проходом 29 августа 2026 — сличением всех 360 карточек ступени, и
 * такой слички в проверках нет.
 */
 ...
 */
/**
 * Ссылка на наш урок, названная НОМЕРОМ, а не именем.
 *
 * Замечанием, не ошибкой: бывает и законная речь о числе уроков («в этом модуле
 * девять уроков»), а образец её от ссылки не всегда отличит.
 *
 * ЗАЧЕМ. Уговор редакторов 5 (`docs/beginner-2/stil-obyasneniy.md`, раздел 8):
 * ссылка называет урок его ВИДИМЫМ именем, и именем полным. Ученик номеров
 * уроков не считает — на карте курса стоят названия. У проверки ссылок на имена
 * есть давний брат: она сверяет, существует ли названный урок, — а ссылку
 * номером не видит вовсе, потому что имени в ней нет.
 *
 * Повод. Номер ставят от безысходности, когда урока ещё нет в коде, и он
 * остаётся навсегда: модуль 3 ссылался «в шестом уроке этого модуля» уже после
 * того, как урок 6 был написан. А редактор модуля 6 нашёл, что заглушки
 * НЕНАПИСАННЫХ уроков 3–9 уже содержат «в уроках 2, 3 и 4» — то есть нарушение
 * уйдёт на экран как есть, если его не поймать заранее.
 */
/*
 * ОСТОРОЖНО С ГРАНИЦАМИ СЛОВА: первая редакция этого образца была МЁРТВОЙ.
 *
 * В ней стояло `\bв\s+урок…`, а `\b` в JavaScript считает словом только
 * `[A-Za-z0-9_]` — кириллицы он не знает вовсе, и `\bв` не совпадает никогда.
 * Проверка молчала бы на всём курсе, и молчание выглядело бы как чистота.
 *
 * Тот же капкан в этом проекте ловил уже дважды: поиск калек, где «дедлайн» не
 * находился ни разу, и проверка ссылок на модули, где `(?:модул|урок)\w*` не
 * находило ничего. Правило проекта поэтому и требует испытывать новую проверку
 * нарочно испорченным содержанием: чистый отчёт бывает и у мёртвой.
 *
 * Границы теперь заданы пробелом и началом строки, а хвост слова закрыт
 * отрицательным просмотром по кириллице.
 */
const SSYLKA_NOMEROM =
  /(?:^|[\s(«"'])в\s+урок[еаих]{1,2}\s+\d|(?:^|[\s(«"'])(?:перв|втор|треть|четв[её]рт|пят|шест|седьм|восьм|девят|десят)[а-яё]{0,3}\s+урок[еаих]{1,2}(?![а-яё])/i;

function checkSsylkaNomerom(course: Course): void {
  for (const mod of course.modules) {
    for (const lesson of mod.lessons) {
      for (const kusok of kuskiUroka(mod, lesson).kuski) {
        const m = kusok.text.match(SSYLKA_NOMEROM);
        if (!m) continue;
        warnings.push(
          `${course.slug} → ${mod.slug} → ${lesson.slug} (${kusok.rol}): ` +
            `ссылка на урок номером — «${m[0]}». Ученик номеров уроков не считает: ` +
            `назови урок его видимым именем целиком`
        );
      }
    }
  }
 ...
 * уроков, — и банк вопросов не открывали вовсе. Нашёл сборщик 6 сентября 2026:
