...
const NEVIDIMYE = new Set([
  ...
  // `pervyyGolos` — кто читает первую реплику разговора, «муж.» или «жен.». Не
  // текст, а указание озвучке; появилось 5 сентября 2026, когда владелец
  // услышал, что Дана говорит мужским голосом.
  //
  // `golosa` — голос КАЖДОЙ реплики по порядку, поле того же ряда. Заведено
  // 6 сентября 2026 по решению владельца «женские имена с женским голосом,
  // мужские с мужским»: чередование врёт там, где говорят двое мужчин.
  //
  // ЕГО ЗДЕСЬ НЕ БЫЛО СУТКИ, И ЭТО СТОИЛО ЧЕТЫРЁХ ЛОЖНЫХ ОШИБОК. Первая же
  // запись с `golosa` дала `yazyk` четыре срабатывания «разбор урока не увидел
  // строку: muzhskoy» — скрипт принял значения поля за непроверенный русский
  // текст, и отчёт по всему курсу стал красным. Нашёл редактор.
  //
  // Отсюда правило: **заводя поле, впиши его сюда сразу.** Список слепого
  // разбора — такая же часть устройства, как сам тип, и о новом поле он не
  // узнаёт никак.
  "pervyyGolos",
  "golosa",
  ...
  "zvuchat",
  // `znak` — код картинки Twemoji (`1f697`), а не текст. Появился вместе с
  // пошаговым видом урока. Без этой строки `yazyk` считал коды невидимым
  // текстом и дал 61 ошибку на одном модуле: правило «дописав новый вид
  // содержания, пройди по проверкам и спроси у каждой, видит ли она его».
  "znak",
  /*
   * `stroki` и `slovo` у задания-сетки — латиница, а не текст для редактора.
   * `stroki` это ячейки поля («CATX», «XDOG»), `slovo` — искомое английское
   * слово. Русское в этом задании одно, `podpis`, и подробный разбор его уже
   * берёт.
   *
   * Без этих двух строк `npm run yazyk` дал 24 ошибки одной породы — «разбор
   * урока не увидел строку» — сразу в двух модулях, как только сетку впервые
   * поставили в уроки. Тот же случай, что был со `znak`: правило «дописав
   * новый вид содержания, пройди по проверкам и спроси у каждой, видит ли она
   * его».
   */
  "stroki",
  "slovo",
